...
class DictRecord(object):
	def __eq__(self,other):
		if isinstance(other, DictRecord):
			if self._model == other._model:
				if self._id == other._id:
					return self._value == other._value
				else:
					raise Exception('self id: %s not eq other id: %s' % (self._id,other._id))
			else:
				raise Exception('self model: %s not eq other model: %s' % (self._model,other._model))
		else:
			raise Exception('instance self: %s not eq instance other : %s' % (type(self),type(other)))
	
	def create(self, model, ci, record):
		self._id = str(id(record))
		self._model = model
		self._value = {}
		for key in filter(lambda x: ci[x] not in ('one2many', 'many2many'),record.keys()):
			self._value[key] = record[key]
		
	def copy(self):
		c = DictRecord()
		c._id = self._id
		c._model = self._model
		c._value = self._value.copy()
		return c
	...
	def diff(self,other):
		...
		if self != other:
			for key in self._value.keys():
				if key in other._value and self._value != other._value:
					res.setdefault(self._id,{}).setdefault('__update__',{})[key] = other._value[key]
			
			s_keys = set(self._value.keys()) 
			o_keys = set(other._value.keys())

			for key in o_keys-s_keys:
				res.setdefault(self._id,{}).setdefault('__insert__',{})[key] = other._value[key]

			for key in s_keys-o_keys:
				res.setdefault(self._id,{}).setdefault('__delete__',{})[key] = self._value[key]
		return res
	
	def apply(self,diff):
		for key in diff.keys():
			print('key:',key)
			if key in ('__update__','__insert__'):
				self._value[key] = diff[key]
			elif key == '__delete__':
				del self._value[key]
...
class ListRecords(object):
	def __init__(self,records = None):
		self._value = []
		if records:
			self._value.extend(map(lambda x: x._id,records))
		
	def append(self,record):
		self._value.append(record._id)
	
	def diff(self,other):
		res = {}
		s_keys = set(self._value)
		o_keys = set(other._value)
		for key in o_keys-s_keys:
			res.setdefault(self._id,{}).setdefault('__append__',{})[key] = other._value[key]

		for key in s_keys-o_keys:
			res.setdefault(self._id,{}).setdefault('__remove__',{})[key] = self._value[key]
		
		return res
	
ci = {'a':'varchar', 'b':'date','c':'many2many','d':'one2mmany','e':'selection'}

r1 = {'a':'Hello','b':'2022-06-06', 'c':[],'d':[]}
r2 = {'b':'2022-06-06', 'c':[],'d':[],'e':'ss'}

v1 = DictRecord('a.b',ci,r1)
v2 = DictRecord('a.b',ci,r2,v1._id)
d1 = v1.diff(v2)
print('diff-1:',d1)
v1.apply(d1)
d2 = v1.diff(v2)
