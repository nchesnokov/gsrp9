...
def _download_imodules(self,path,module,imodels,registry,ext='csv'):
	afile = opj(path,module,'tests','annotation-1.csv')
	if  os.path.exists(afile):
		a = open(opj(path,module,'tests','annotation-1.csv'),'a')
	else:
	...
	for model in imodels.keys():	
		...
		if len(records) > 0:
			...
			am = open(opj(path,module,'tests',c,m._table+'_'+'.' + ext),'w')
			#print('MFS:',mfs)
			for row in records:
				for key in row.keys():
					if key == 'id':
			...
			if ext == 'csv':
				...
				with open(opj(path,module,'tests',c,m._table+'_'+'.yaml'),'w') as outfile:
					yaml.dump(records, outfile, Dumper, default_flow_style=False)
			
			#aw.writerow({'model':m._name,'file':opj('demo',c,m._table+'_'+k+'.' + ext)})
			aw.writerow({'model':m._name,'file':opj('tests',c,m._table+'_'+'.' + ext)})
	a.close()
...
def _download(self,path,module,models,registry,ext='csv'):
	#pool = self._pool
	#pool = models
	_remove_dirs([opj(path,module,'tests','data'),opj(path,module,'tests','examples'),opj(path,module,'tests','cust'),])
	...
	for key in models.keys():
		...
		if len(records) >= 0:
			...
			_logger.info('GenTests write file: %s - records:%s'  % (opj(path,module,'tests',c,model._table+'.' + ext),len(records)));
			am = open(opj(path,module,'tests',c,model._table+'.' + ext),'w')
			for row in records:
				for key in row.keys():
					if key == 'id':
						continue
			...
			if ext == 'csv':
				...
				with open(opj(path,module,'tests',c,model._table+'.yaml'),'w') as outfile:
					yaml.dump(records, outfile, Dumper=Dumper, default_flow_style=False)
			
			aw.writerow({'model':model._name,'file':opj('tests',c,model._table+'.' + ext)})
	a.close()
