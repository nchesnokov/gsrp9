            ...
            column>
              <v-form v-model="valid" ref="form" lazy-validation>
                <template v-if="Object.keys(actions).length > 0">
						<h3>Actions</h3>
						<v-layout row>
							<v-btn v-for="(action,akey) in actions" :key="akey" fab dark small color="purple darken-2" @click="do_action(akey,item.__data__)">{{ action.icon }}</v-btn>
						</v-layout>
                </template>
				<template v-for="field in formfields">
					...
					<template v-else-if="meta.columns[field].type == 'many2one'">
						<v-layout row v-show="visible(field)">
							<v-text-field v-model="item.__data__[field].name" :rules="rules[field]" :label="meta.columns[field].label" :readonly="readonly(field)" :required="required(field)" @change="m2o_cache(item,field)" clearable></v-text-field>
					...
					<template v-else-if="meta.columns[field].type == 'related'">
						<v-layout row v-show="visible(field)">
							<v-text-field v-model="item.__data__[field].name" :rules="rules[field]" :label="meta.columns[field].label" :readonly="readonly(field)" :required="required(field)" @change="related_cache(item,field,meta.columns[field].relatedy)" clearable></v-text-field>
					...
					<template v-else-if="meta.columns[field].type == 'binary'">
						<v-card v-show="visible(field)">
							...
							<v-responsive>
								<v-img class="preview" :src="item.__data__[field] ? item.__data__[field]:''" width="128px" height="128px"></v-img>	
							</v-responsive>
							<v-card-actions>
								<div class="image-preview">
									<input type="file" @change="previewImage" :accept="meta.columns[field].accept" :name="field" :disabled="readonly(field)"/>
									<v-icon @click="clearImage(item,field)" v-if="item.__data__[field] != null && item.__data__[field].length > 0"> clear</v-icon>
								</div>							
							</v-card-actions>
						</v-card>
					</template>
						...
						<v-tabs v-model="active">
							...
							<v-tabs-items v-model="active">
								<v-tab-item  v-for="(o2mfield,index) in o2mfields" :key="index" :value="'tab-' + o2mfield">
									<gsrp5-o2mlist-view :metas="metas" :meta="metas[meta.columns[o2mfield].obj].meta" :model="meta.columns[o2mfield].obj" :views="metas[meta.columns[o2mfield].obj].views" :items="item.__o2m_containers__[o2mfield]" :rel="meta.columns[o2mfield].rel" :mode="(mode == 'readonly' || readonly(o2mfield)) ? 'readonly':mode" role="role" :o2mfield="o2mfield" :root="root" :guid="guid" :container="o2mfield + '.' + item.__path__" :parent="item.__path__"></gsrp5-o2mlist-view>
								</v-tab-item>
							</v-tabs-items>
						</v-tabs>					
			        </v-flex>
