...
export default {
	name:'gsrp5-form-view',
	props: {'metas':Object,'model':String,'path':{'default':null},'modal':{'default':false},'mode':String},
	data: () => ({
	  a: {c:{},v:{},r:{},d:{}},
	  swipeDirection: 'None',
	  meta: null,
   ...
   importedData: {
    ...
    methods: {
        ...
        previewImage: function(event) {
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = (e) => {
                    ...
                    this.$set(this.item.__data__,input.name,e.target.result);
                    console.log('EVENT:',e);
                    let r = {'path':this.item.__path__,'key':input.name,'value':this.item.__data__[input.name],'context':{}};
        ...
        _buildItem (){
 			let item = {__data__:{},__o2m_containers__:{},__m2m_containers__:{}}
 			for(let i = 0,field = '',fields = Object.keys(this.view.columns); i < fields.length;i++ ){
 				...
 				switch(this.meta.columns[field].type){
					...
					case 'related':
						...
						break;
					case 'one2many':
						item.__o2m_containers__[field] = [];
						break;
					case 'many2many':
						break;
					case 'datetime':
						item.__m2m_containers__[field] = null;
						break;
					case 'date':
        ...
        save () {
          ...
          if (this.mode == 'lookup' || this.mode == 'edit') {
            ...
            //this.send('onSave',['models',this.model,'modify',{'records':record}]);
            this.send('onSave',['_cache','save',this.guid,{}]);
          } else {
            record = this.prepareForSave(this.meta,this.item,'new')
            //if (Object.keys(record).length > 0) this.send('onSave',['models',this.model,'create',{'records':record}]);
	   ...
	   onSave(msg){
		...
		if (this.modal) {
			this.dialog = false;
			this.send('onUpdate',['models',this.model,'read',{'fields':this.fields,'ids':msg[0]}]);
		  }
		  }
		  if (action == 'no chache') this.$snotify.info('No cache');
		 },
	  onInitialize(msg){
		  console.log('item:',msg[0]);
	  ...
	  onUpdate(msg){
		...
		switch(this.role){
			case 'many2one-new':
				...
				////this.$store.commit('m2onew',v);
				break;
			case 'related-new':
				v['obj'] = this.obj;
				v['rel'] = this.rel;
				v['v'] = {id:msg[0].id,name:msg[0][rec_name],idx:this.idx,item:this.item_find};
				////this.$store.commit('relatednew',v);
				break;
			}
		this.send('.ws-log',['_cache','commit',this.guid,{'action':'commit work'}]);
		},
	onLoadMeta: function(msg){
		if (this.modal){
			...
			//this.$store.commit('attach',{'m':this.metas[this.model].views.form.viewname,'v':this});
		},
	onOpen(msg){
		if (this.guid == null) this.guid = msg[0];
		switch(this.mode){
			case 'new':
				if (this.modal) this.send('onLoadMeta',['ui','get_view_by_name_v2',{'name':'view.'+this.model+'.form'}]);
				else {
					this.send('onInitialize',['_cache','initialize',this.guid,{'model':this.model,'view':'form','context':{}}]);
					this.$set(this,'root',`${this.$options.name}:${this._uid}`);
					//this.$store.commit('attach',{'m':this.metas[this.model].views.form.viewname,'v':this});
					}
				break;
			...
			case 'edit':
				...
				//this.$store.commit('attach',{'m':this.metas[this.model].views.form.viewname,'v':this});
				this.$set(this,'root',`${this.$options.name}:${this._uid}`);
				this.send('onRead',['models',this.model,'read',{'fields':this.fields,'ids':this.$parent.selected[0].id,'context':{'cache':this.guid}}]);
			}
		},
	do_action(name,item){
	...
	dataRowContainer(containers,parent){
		for(let k in containers) {
			//console.log('containers:',k,containers[k],containers);
	...
	render: function(createElement){
	    if (!this.modal) {
	      return Vue.complile(this.$options.template);
	    } else {
	      return Vue.complile('<v-dialog v-model="dialog" scrollable  persistent max-width="900px">'+this.$options.template+'</v-dialog>');
	    }			
	},
