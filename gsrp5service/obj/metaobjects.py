...
_objects = ['action','controller', 'dashboard', 'dialog', 'form', 'link', 'metaobjects', 'model', 'query', 'report', 'trigger', 'view', 'wizard']
		...
		if '__module__' in attrs:
			...
			if len(_m) >  2 and _m[2] not in _objects:
				...
				if _m[0] == 'gsrp5service':
					if _m[1] == 'obj':
						_module = reduce(lambda x,y: x + '.' + y,attrs['__module__'].split('.')[2:3])
						_obj = 	attrs['__module__'].split('.')[3]
